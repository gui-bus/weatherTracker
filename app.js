...
function displayWeather(data) {
  ...
  sunriseTime.innerHTML = `<img class="sun-img" src="src/assets/sunrise.png" alt="Ícone do nascer do sol" /><br> ${sunriseTimeFormatted}`;
  ...
  sunsetTime.innerHTML = `<img class="sun-img" src="src/assets/sunset.png" alt="Ícone do pôr do sol" /><br>${sunsetTimeFormatted}`;

  // !City data | Weather info
  cityName.innerHTML = `Confira abaixo os dados climáticos atuais de <span class="fw-bold">${name}</span>,`;
  cityCountry.innerHTML = `<span class="fw-bold">${cityCountryTranslations[country]}</span>.` ;
  weatherIcon.src = `./src/assets/${icon}.svg`;
  weatherMain.innerHTML = `<span class="fw-bold">${weatherTranslations[main]}</span>`;
  ...
  const dateFormatted = `${
    daysOfWeek[cityDate.getUTCDay()]
  }, ${cityDate.getUTCDate()} de ${cityDate.toLocaleDateString("pt-BR", {
    month: "long",
  })} de ${cityDate.getUTCFullYear()}`;
  ...
  cityUtc.textContent = `${dateFormatted} - Hora local ${timeFormatted} (GMT${
    timezoneOffset > 0 ? "+" : ""
  }${timezoneOffset / 3600})`;

  //!Temperature
  currentTemperature.textContent = `${Math.round(temp)}°C`;
  fellsLikeTemperature.textContent = `${Math.round(
    feels_like
  )}°C `;
  ...
  // !Coords
  cityLatitude.textContent = `${lat.toFixed(
    2
  )}° /`;
  cityLongitude.textContent = ` ${lon.toFixed(
    2
  )}°`;
}
